...
A4J.AJAX.Submit =  function(formId, event, options, old_options ) 
{
  //Exigen Fix for IPBDEV-5582 
  //Check for old-style code:  
  if (formId == '_viewRoot' || old_options != null)
  {
    var correctFormId = event;
    var correctEvent = options;
    var correctOptions = old_options;
    
    formId = correctFormId;
    event = correctEvent;
    options = correctOptions;    
  }
  //end of Exigen Fix IPBDEV-5582 
...
A4J.AJAX.processResponse = function(req) {
		...
		//
              ...
              //~IE9: LOG object is not initalized ~ LOG.debug("Hidden JSF state fields: "+idsSpan);
        	  if(idsSpan != null){
...
A4J.AJAX.replacePage = function(req){
						...
						window.setTimeout(function() {
							...
							try {  	
								...
								if(isIE&&Sarissa._SARISSA_IS_IE9) {
									window.document.write(responseText.replace(/(<script(?!\s+src=))/igm, "$1 defer "));
								} else {
